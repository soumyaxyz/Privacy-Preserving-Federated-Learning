...
from utils.training_utils import get_device, save_loss_dataset, load_loss_dataset, train_shadow_model, get_device, print_info, save_model,train, test, load_model as load_saved_weights
from utils.datasets import Loss_Label_Dataset, get_datasets_details, load_datasets, load_dataloaders
...
class Membership_inference_attack_instance:
    def __init__(self, class_id, 
        ...
        self.batch_size             = 32
        self.device                 = device
        self.save_attack_dataset    = False
        self.save_attack_model      = False
    ...
    def run_membership_inference_attack(self):
        ...
        loss, accurecy = test(self.attack_model, self.attack_testloder, device=self.device, is_binary=True)
...
def main():
    ...
    parser.add_argument('-l', '--load_attack_dataset', action='store_true', help='Instead of building attack dataset, load pre-existing attack dataset from disc')
